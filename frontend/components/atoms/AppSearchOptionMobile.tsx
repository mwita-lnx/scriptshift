      ...
      >
        ...
        <AppSearchOptionWrapperMobile
          ...
          handleOnBack={() => {
            setStep((prev) => prev - 1);
            if (step === 1) {
              ({ type: DATA_ACTION_TYPES.SET_LOCATION, payload: '' });
              setStep(1);
              onClose();
            }
            if (step === 2) ({ type: DATA_ACTION_TYPES.RESET_DATES });
            if (step === 3) ({ type: DATA_ACTION_TYPES.RESET_GUESTS });
          }}
        >
          {step === 1 && (
            <Step1
          ...
          {step === 3 && (
            <Step3
              handleOnNext={() => {router.push( '/search')    
      ...
      <div>
        <div className="flex py-4 border-b border-gray-200 border-opacity-70">
          ...
          <AppCounter
            value={1}
            maxValue={16}
            onIncrease={() => ({ type: DATA_ACTION_TYPES.INCREASE_ADULTS })}
            onDescrease={() => ({ type: DATA_ACTION_TYPES.DECREASE_ADULTS })}
          />
        </div>
      </div>
      <div>
        <div className="flex py-4 border-b border-gray-200 border-opacity-70">
          ...
          <AppCounter
            value={2}
            maxValue={5}
            onIncrease={() => ({ type: DATA_ACTION_TYPES.INCREASE_CHILDREN })}
            onDescrease={() => ({ type: DATA_ACTION_TYPES.DECREASE_CHILDREN })}
          />
        </div>
      </div>
      <div>
        <div className="flex py-4">
          ...
          <AppCounter
            value={3}
            maxValue={5}
            onIncrease={() => ({ type: DATA_ACTION_TYPES.INCREASE_INFANTS })}
            onDescrease={() => ({ type: DATA_ACTION_TYPES.DECREASE_INFANTS })}
          />
        </div>
      </div>
      <div className="fixed bottom-0 left-0 right-0 grid items-center grid-cols-2 p-4 text-lg bg-white border-t border-gray-200 gap-x-4">
